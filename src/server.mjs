...
export async function createInboxServer({ projectRoot }) {
  ...
  const server = createServer(async (request, response) => {
    ...
    const entryApiMatch = /^\/api\/entries\/(INBOX-\d{8}-\d{6}-[a-z0-9]{1,12})$/.exec(pathname)
    if (entryApiMatch && request.method === 'GET') {
      try {
        respondJson(response, 200, await readInboxDetails(config, entryApiMatch[1]))
      } catch (error) {
        respondJson(response, 404, { error: error instanceof Error ? error.message : 'Unable to read the inbox item.' })
      }
      return
    }
