...
test('README exposes copyable installation shortcuts', async () => {
  ...
  assert.match(readme, /\[Install globally with Git\]\(#install-globally-with-git\)/)
  ...
  assert.match(readme, /### Install globally with Git/)
  ...
  assert.match(readme, /then starts the inbox in the same session/)
  assert.match(readme, /non-Git snapshot, typically at\n`~\/\.codex\/skills\/project-inbox`/)
  assert.match(readme, /update the snapshot later, repeat the installation request/)
  assert.match(readme, /If\nthe installed skill is not discovered, restart Codex/)
  assert.doesNotMatch(readme, /In the next turn/)
...
test('the capture page keeps its small controls contextual', async () => {
  ...
  assert.match(genericPage, /<button id="submit" type="button">save ↵<\/button>/)
  assert.match(genericPage, /<button class="secondary" id="cancel-reuse" type="button" hidden>cancel<\/button>/)
  assert.match(genericPage, /<button id="submit"[^>]*>save ↵<\/button>\s*<button class="secondary" id="cancel-reuse"/)
  assert.doesNotMatch(genericPage, /class="message-heading"/)
  assert.match(genericPage, /setStatus\('Source loaded\.', 'success'\)/)
  assert.doesNotMatch(genericPage, /save as new capture/)
  assert.doesNotMatch(genericPage, /Saving creates a new capture/)
